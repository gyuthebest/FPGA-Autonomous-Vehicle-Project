...
module preprocessor (
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (valid_s0) begin
                if (timeout_mask_1s || consistency_mask_1s_approach_speed || consistency_mask_20s_approach_speed || (win == M-1) || (sim_data_in.situation == 3'b001)) begin // 20s 추�?
    ...
    always_ff @(posedge clk) begin
        ...
        if(!rst_n) begin
            ...
            if(valid_s0) begin
                ...
                clk_cnt                 <= '0;
                if (timeout_mask_1s || consistency_mask_1s_approach_speed || consistency_mask_20s_approach_speed || (win == M-1) || (sim_data_in.situation == 3'b001)) //20s추�?
                    win <= '0;
                else 
                    win <= win + 1;
            end
