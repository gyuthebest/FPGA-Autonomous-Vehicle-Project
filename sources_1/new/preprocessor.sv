...
module preprocessor (
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (valid_s0) begin
                if (timeout_mask_1s || consistency_mask_1s_approach_speed || consistency_mask_approach_speed || (win == M-1) || (sim_data_in.situation == 2'b01)) begin
    ...
    always_ff @(posedge clk) begin
        ...
        if(!rst_n) begin
            ...
            if(valid_s0) begin
                ...
                clk_cnt                 <= '0;
                if (timeout_mask_1s || consistency_mask_1s_approach_speed || consistency_mask_approach_speed || (win == M-1) || (sim_data_in.situation == 2'b01)) win <= '0;
                else win <= win + 1;
            end
