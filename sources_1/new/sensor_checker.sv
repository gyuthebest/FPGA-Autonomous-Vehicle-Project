...
module each_sensor_check #(
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if(valid_s1) begin
                range_cnt <= (raw_range == 1'b1) ? range_cnt + RANGE_U : ((range_cnt < RANGE_D) ? '0 : range_cnt - RANGE_D);
            end
        end
    end
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (valid_s1) begin
                if (raw_stuck == 2'b00) stuck_cnt <= (stuck_cnt < JUMP_D) ? '0 : stuck_cnt - JUMP_D;
                else if (raw_stuck == 2'b01) stuck_cnt <= stuck_cnt + JUMP_U;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (valid_s1) begin
                ...
                else if (raw_stuck == 2'b01) stuck_cnt <= stuck_cnt + STUCK_U;
                else stuck_cnt <= stuck_cnt;
            end
        end
    end
    ...
    // timeout check
    assign raw_timeout = (clk_cnt == UPDATE_CLK_X2 - 1);
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (raw_timeout == 1'b1) timeout_cnt <= timeout_cnt + TIMEOUT_U;
            else timeout_cnt <= (timeout_cnt < TIMEOUT_D) ? '0 : timeout_cnt - TIMEOUT_D;
        end
    end
