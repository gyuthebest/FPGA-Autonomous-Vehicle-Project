...
module risk_types (
    ...
    always_comb begin
        ...
        if (sensor_data_in.approach_speed <= 0)
            ...
            // Emergency 경계는 TTC 1.4초다.  1.4 를 시프트 합으로 근사한다.
            //   1 + 1/4 + 1/8 + 1/32 = 1.40625  (오차 +0.45 %)
            // 이전에는 (approach_speed >> 1) 만 더해 1.5초였다.
            if (sensor_data_in.distance <= (sensor_data_in.approach_speed
                                            + (sensor_data_in.approach_speed >> 2)
                                            + (sensor_data_in.approach_speed >> 3)
                                            + (sensor_data_in.approach_speed >> 5)))
                collision_risk = 3'b100; // Emergency. TTC <= 1.4초 (근사 1.40625)
