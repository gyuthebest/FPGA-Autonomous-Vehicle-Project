...
module sensor_reliability #(
    parameter int unsigned CLK_FREQ_HZ    = 88888000,
    ...
    // One timeout evidence tick per two expected 20 Hz sample periods
    // (100 ms independent of the configured PL clock). TN consecutive ticks confirm a
    // continuous missing-sample fault after 1 s with the default parameters.
    ...
    // gyro consistency 임계값.
    // 기준값 pred_gyro_*_1 = (incline 차분) * C_GYR(3574) 이고, incline LSB가
    // 0.01 deg이므로 기준값은 3574 단위로만 움직인다.  즉 양자화만으로
    // +-1787의 잔차가 항상 발생한다.  이전 값 270은 이 양자화 바닥의 1/6에
    // 불과해, 비트폭을 고쳐도 선회 중에는 consistency가 구조적으로 통과할 수
    // 없었다.  0.5 rad/s 정상 선회 재생에서 측정한 잔차 최대 6752에 여유를
    // 더해 7300으로 둔다.  검출 민감도는 7300/S_GYR(1024) = 7.1 (0.0071 rad/s
    // = 0.41 deg/s) 상당으로 여전히 충분하다.
    localparam int TH_GYR       = 7300;
    ...
    // Shared periodic timeout phase.  The former direct use of preprocessor
    // clk_cnt produced only one equality pulse during an indefinitely missing
    // sample, so TIMEOUT_N > 1 could never be reached continuously.
    always_ff @(posedge clk) begin
        if (!rst_n || valid_s1) begin
            timeout_phase_cnt <= '0;
        end
        else if (timeout_phase_cnt >= UPDATE_CLK_X2 - 1) begin
            timeout_phase_cnt <= '0;
        end
        else begin
            timeout_phase_cnt <= timeout_phase_cnt + 32'd1;
        end
    end
    ...
    // DISTANCE_SENTINEL means "no tracked object", not a 200 m physical
    // measurement.  There is no dynamics reference to compare in this state.
    assign consistency_mask_7 = untrusted_approach_speed |
                                (sensor_data_in.distance == 15'd20000);
    assign consistency_mask_8 = untrusted_distance;

    //==========================================================================
    // 관계식 6/7/8 의 gyro 비교항
    //==========================================================================
    // (1) 중점 보정
    //     기준값 pred_gyro_*_1 = (incline[t] - incline[t-1]) * C_GYR 은 구간
    //     [t-1, t] 의 **평균** 각속도다.  이것을 순시값 gyro[t] 와 비교하면
    //     각가속도가 큰 구간에서 잔차가 커진다.  대응하는 값은 중점
    //     (gyro[t] + gyro[t-1]) / 2 다.  나눗셈을 피하려고 합을 그대로 쓰고
    //     S 를 절반(S_GYR/2)으로 준다.  gyro[t-1] = gyro[t] - delta_gyro 이므로
    //     합 = 2*gyro[t] - delta_gyro 로 얻는다.
    //
    //     실측 근거: gyro 를 k 표본 밀어 정렬을 바꿔 본 결과 관계식 8 의
    //     임계초과율이 k=-1 36.6% / k=0 24.8% / k=1 30.9% 로 k=0 이 정수
    //     정렬 중 최적이고, 반 표본 중점이 12.7% 로 그보다 더 낮았다.
    //
    // (2) roll/pitch 부호
    //     CARLA 의 rotation.roll / rotation.pitch 는 IMU gyro 의 x/y 축과
    //     부호 규약이 반대다.  yaw 만 일치한다.  gyro 와 delta_incline 의
    //     상관계수 실측(pl_capture_20260814_183046): x -0.864 / y -0.968 /
    //     z +0.998.  그래서 관계식 6/7 만 부호를 뒤집고 8 은 그대로 둔다.
    //     (8 을 뒤집으면 임계초과율이 3.2% -> 63.0% 로 악화된다.)
    //
    // 폭: 합의 범위는 +-64000 이라 28비트 안에 충분히 들어간다.
    //     consistency_check 안에서 S=512 로 곱해도 최대 +-32.8M 이므로
    //     32비트 문맥에서 wrap 되지 않는다.  기준값 폭 28비트는 그대로 둔다.
    logic signed [27:0] gyro_x_w,  gyro_y_w,  gyro_z_w;
    logic signed [27:0] dgyro_x_w, dgyro_y_w, dgyro_z_w;
    logic signed [27:0] gyro_cmp_x, gyro_cmp_y, gyro_cmp_z;

    assign gyro_x_w  = sensor_data_in.gyro_x;
    assign gyro_y_w  = sensor_data_in.gyro_y;
    assign gyro_z_w  = sensor_data_in.gyro_z;
    assign dgyro_x_w = processed_data_in.delta_gyro_x;
    assign dgyro_y_w = processed_data_in.delta_gyro_y;
    assign dgyro_z_w = processed_data_in.delta_gyro_z;

    // -(2*gyro - delta) = delta - 2*gyro   (부호 반전 포함)
    assign gyro_cmp_x = dgyro_x_w - (gyro_x_w <<< 1);
    assign gyro_cmp_y = dgyro_y_w - (gyro_y_w <<< 1);
    // yaw 는 부호가 맞으므로 반전하지 않는다.
    assign gyro_cmp_z = (gyro_z_w <<< 1) - dgyro_z_w;
    ...
    always_comb begin
        ...
        // 20000 with zero closing speed is the protocol's "no target" state.
        // It is not a held 200 m physical measurement and must not become a
        // distance sensor fault because of predictor/history residue.  Keep
        // the independent transport timeout, however: "no target" must not
        // hide loss of the complete PS-to-PL sample stream.
        if ((sensor_data_in.distance == 15'd20000) &&
            (sensor_data_in.approach_speed == 13'sd0)) begin
            reliability.distance = pack_ch(
                1'b0, 1'b0, 1'b0, 1'b0, 1'b0, timeout_err[CH_DIST]
            );
        end
    end
    ...
    each_sensor_check #(
        ...
        .clk(clk), .rst_n(rst_n), .valid_s1(valid_s1), .clk_cnt(timeout_phase_cnt),
        ...
        // Distance is testable whenever a tracked target has a non-zero
        // closing speed.  Using delta_approach_speed made this path
        // impossible: DIST stuck required |delta| >= 20 while the closing-
        // speed jump checker declared the reference untrusted at |delta| > 10
        // and immediately masked the stuck checker.
        .trig_val_1         (sensor_data_in.approach_speed),
        .trig_val_2         ('0),
        .situation          (situation),
    ...
    each_sensor_check #(
        .WIDTH(12), .DW(13), .TW(15), .CHANNEL_TYPE_1(0), .CHANNEL_TYPE_2(1),
        .UPDATE_CLK_X2(UPDATE_CLK_X2), .DROP_N(DROP_N), .HISTORY(HISTORY_LEN),
    ...
    each_sensor_check #(
        .WIDTH(16), .DW(17), .TW(17), .CHANNEL_TYPE_1(0), .CHANNEL_TYPE_2(1),
        .UPDATE_CLK_X2(UPDATE_CLK_X2), .DROP_N(DROP_N), .HISTORY(HISTORY_LEN),
    ...
    // 관계식 1
    consistency_check #(
        .WIDTH(21), .S(S_DIST), .CONSISTENCY_THRESHOLD(TH_DIST),
        ...
        .sensor_data      ({{6{1'b0}}, sensor_data_in.distance}),
    ...
    // 관계식 6
    consistency_check #(
        // pred_gyro_x_1이 28비트이므로 비교 폭도 28비트여야 한다.
        // 16비트로 두면 예측값이 wrap되어 정상 선회가 고장으로 판정된다.
        // S 가 절반인 것은 gyro_cmp_x 가 중점의 '합'이기 때문이다.
        .WIDTH(28), .S(S_GYR/2), .CONSISTENCY_THRESHOLD(TH_GYR),
        .CONSISTENCY_U(U_CONS), .CONSISTENCY_D(D_CONS), .CONSISTENCY_N(N_CONS)
    ) u_cons_1_gyro_x (
        .clk(clk), .rst_n(rst_n), .valid_s1(valid_s1),
        .timeout_mask_2s  (tm2[CH_GX]),
        .consistency_mask (consistency_mask_2),
        .sensor_data      (gyro_cmp_x),
    ...
    // 관계식 7
    consistency_check #(
        // pred_gyro_y_1이 28비트이므로 비교 폭도 28비트여야 한다.
        // 16비트로 두면 예측값이 wrap되어 정상 선회가 고장으로 판정된다.
        // S 가 절반인 것은 gyro_cmp_y 가 중점의 '합'이기 때문이다.
        .WIDTH(28), .S(S_GYR/2), .CONSISTENCY_THRESHOLD(TH_GYR),
        .CONSISTENCY_U(U_CONS), .CONSISTENCY_D(D_CONS), .CONSISTENCY_N(N_CONS)
    ) u_cons_1_gyro_y (
        .clk(clk), .rst_n(rst_n), .valid_s1(valid_s1),
    ...
    // 관계식 8
    consistency_check #(
        // pred_gyro_z_1이 28비트이므로 비교 폭도 28비트여야 한다.
        // 16비트로 두면 예측값이 wrap되어 정상 선회가 고장으로 판정된다.
        // S 가 절반인 것은 gyro_cmp_z 가 중점의 '합'이기 때문이다.
        .WIDTH(28), .S(S_GYR/2), .CONSISTENCY_THRESHOLD(TH_GYR),
        .CONSISTENCY_U(U_CONS), .CONSISTENCY_D(D_CONS), .CONSISTENCY_N(N_CONS)
    ) u_cons_1_gyro_z (
        .clk(clk), .rst_n(rst_n), .valid_s1(valid_s1),
