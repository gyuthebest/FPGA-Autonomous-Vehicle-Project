...
module each_sensor_check #(
    ...
    always_comb begin
        ...
        // Both conditions must hold.  The sign-flip count alone cannot
        // distinguish a noisy sensor from a healthy one: a normal IMU
        // oscillates about its mean and flips the sign of its first
        // difference nearly every sample even when the excursion is a single
        // LSB.  On fault-free driving the magnitude term never fired (0.00%
        // on every channel) while the flip term accounted for 100% of the
        // noise false positives.  Requiring magnitude AND flip rate keeps the
        // "rapidly alternating" evidence but demands that the alternation be
        // physically large enough to matter.
